...
**claude-ltm applicability.** ⚠️ **N/A — there is no client/browser session.** claude-ltm is a plugin, not a web app; there is no request/response wire to carry state on, no cookie, no header auth. The read-only localhost viewer (`viewer/`) is stateless per request over the store, and holds nothing session-scoped.

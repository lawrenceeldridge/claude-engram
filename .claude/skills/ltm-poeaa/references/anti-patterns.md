...
scope; others combine in ways that produce well-known anti-patterns. claude-ltm adds a
...
3. (claude-ltm–specific) it breaks the Hexagonal dependency direction, the CQRS hot-path
...
| Rich Domain Model | plain-data + Functional Core | same seam | Contradictory mental models; claude-ltm chose data + pure functions |
...
| **Event** bus / pub-sub | detached capture / durable Command queue | capture | claude-ltm has no *Event* bus (no publishers/subscribers, no broadcast). A durable **Command** queue (`MemoryBus`, opt-in) is permitted — one handler, retry/dead-letter, not pub-sub. See `ltm-defaults.md` §§ Distribution / Offline Concurrency |
...
worker. See `ltm-defaults.md` § Base (fail-open) and `.claude/rules/02-architecture/02-hooks-and-budgets.md`.
...
classes — claude-ltm is single-store, single-context. See `catalog/05-or-metadata.md` § Repository
...
**Fix.** claude-ltm uses one short-lived connection + explicit transactions + content-hash
...
## claude-ltm–specific anti-patterns (budget / fail-open / dependency direction)
...
§ claude-ltm substitute and `ltm-defaults.md` § Offline Concurrency.
...
This was a real regression fixed in commit `b30889a`. See `ltm-defaults.md` § Offline Concurrency.
...
**Fix.** claude-ltm is one package, one context, one set of defaults. See `single-package.md`.

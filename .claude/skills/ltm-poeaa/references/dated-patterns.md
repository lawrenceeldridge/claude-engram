...
- **Dishan M.** ([Medium article](https://dishanm.medium.com/enterprise-application-architecture-patterns-the-immutable-laws-of-change-48716758da84))
  ...
  Record.
- **Modern reality for claude-ltm.** claude-ltm deliberately runs on the **standard library**
...
| Pattern | Category | Why dated | Modern equivalent (claude-ltm) |
|---------|----------|-----------|--------------------------------|
...
| **Active Record** | Data Source | Combines persistence with domain logic; violates SRP | Repository over Data Mapper — the claude-ltm default; **forbidden** |
...
| **Transform View** | Web Presentation | XSLT-era programmatic HTML emission | N/A — claude-ltm renders minimal HTML in the viewer only |
...
| **Implicit Lock** | Offline Concurrency | Was a hand-rolled framework hook | N/A — no offline locking in claude-ltm |
...
| **Serialized LOB** (XML form) | O-R Structural | XML-era opaque-blob persistence | Modern LOB: int8/binary embedding **blobs** stored whole (never filtered inside) — claude-ltm **does** use this; only the *XML* form is dated |
...
## Patterns that are NOT dated (claude-ltm uses them freely)
...
These map directly onto claude-ltm's locked-in defaults in `ltm-defaults.md`.
...
claude-ltm has no ORM and no rich domain — do **not** flag them as "dated", and do **not**
introduce machinery to "complete" them:

- **Domain Model** — claude-ltm has no rich domain; a fact is data. (Deliberate, not a gap.)
- **Unit of Work / Identity Map / Lazy Load** — ORM-session behaviours; claude-ltm uses one
...
See `catalog/03-or-behavioral.md`, `08-offline-concurrency.md`, and `ltm-defaults.md` for the
...
- **Forbidden in claude-ltm** (e.g. Active Record) → high; propose migration in remediation.
...
> claude-ltm — it would put `save()`/`delete()` on the fact rows and couple persistence to
...
Once confirmed, apply the pattern as canonically documented and update `references/ltm-defaults.md`
...
- **Transaction Script** — borderline. Acceptable for a trivial one-shot maintenance CLI
  command; dated for anything with real logic. claude-ltm's `bin/*` scripts stay clean because
  they are thin Composition Roots that delegate to `service.py` / `recall.py` — not transaction
  scripts.
- **Serialized LOB** — dated only in its XML form. claude-ltm's int8/binary embedding blobs are
...
- **Single Table Inheritance** — not in the dated list, but claude-ltm has no hierarchy, so it

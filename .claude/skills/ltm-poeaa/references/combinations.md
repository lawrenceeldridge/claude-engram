...
|--------------|---------------------|-----|
| Repository | Data Mapper | Repository is built *on top of* the mapper (in claude-ltm both are `store.py`) |
...
## claude-ltm's adopted bundles

When claude-ltm reaches for one pattern, it brings the whole bundle. Don't propose any
...
Separated Interface (EmbeddingGateway ABC / Distiller ABC)   ← core/embedding.py, core/distill.py
    ...
    └─ Plugin (get_embedder(cfg) / get_distiller(cfg) — runtime selection)
        └─ Composition Root (bin/* picks the impl from config/env; LTM_DAEMON toggles daemon)
...
- <Pattern A> — already in claude-ltm ✅ / needs introducing
- <Pattern B> — already in claude-ltm ✅ / needs introducing
```

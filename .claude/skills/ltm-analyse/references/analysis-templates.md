...
**PR:** #{N} (if PR-linked) · **Date:** {YYYY-MM-DD} · **Scope:** {layers / modules} · **Analyst:** Claude (ltm-analyse)
...
- Depth: the [`ltm-poeaa`](../../ltm-poeaa/SKILL.md) skill (catalogue, decision trees, anti-patterns)
...
|---------------------|------------------------------|---------|
| `hash,fastembed` | Recall@3, MRR@10 | `python3 bin/ltm eval --backends "hash,fastembed"` |
...
Change Y (float-rescore)
  ...
  +-- depends-on: ltm eval A/B result     — BLOCKING
```
...
| Benchmark before default change | `ltm eval` must confirm no regression | Ships an unmeasured recall regression |
...
**Detection:** {how to detect it early — e.g. ltm eval regression, hook exit code}
...
| 3 | Re-run `ltm eval` if retrieval touched | No metric regression |
...
| Benchmark | Recall quality through the quantised search path | `ltm eval` | `hash` vs `fastembed` |
...
|--------|--------|---------------|
| Recall@3 | >= current default (0.86) | `python3 bin/ltm eval --backends "hash,fastembed"` |
| MRR@10 | no regression vs baseline | same, compare backends |
| bytes/fact | within storage budget | reported by `ltm eval` |

# Recall-Quality Benchmark (`ltm eval`)

claude-ltm has a second test surface beyond correctness: a labelled **recall
...
> A/B'd with `ltm eval` **before** it ships.
...
python3 bin/ltm eval --backends "hash,fastembed"              # stub vs real model
python3 bin/ltm eval --backends "fastembed,fastembed+float"   # isolate int8 loss
python3 bin/ltm eval --backends "fastembed@BAAI/bge-small-en-v1.5,fastembed@BAAI/bge-base-en-v1.5"
...
1. **Always:** `python3 -m unittest discover -s tests` (zero-dep; the 5 optional
   skips are expected).
2. **Optional retrieval smoke:** `python3 bin/ltm eval --backends hash` — cheap,

...
claude-ltm has **no pytest marker system** — no tiers, no `@pytest.mark.integration`.
...
```python
@unittest.skipUnless(os.environ.get("LTM_SLOW_TESTS"), "slow test; set LTM_SLOW_TESTS=1")
...
FastAPI/Citus app with real database and NATS infrastructure. claude-ltm has none

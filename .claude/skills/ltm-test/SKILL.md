---
name: ltm-test
description: claude-ltm test lifecycle — the stdlib unittest suite (write / run / review / audit tests, coverage) plus the recall-quality benchmark (`ltm eval`). Use when writing or reviewing tests for `core/**`, hooks, adapters, the CLI or MCP tools; when asked "run the tests", "write a test for X", "review these tests", "can I delete this test", "check coverage"; or when changing embeddings, ranking, quantisation, fusion, or distillation — those must be A/B'd with `ltm eval` before shipping. Not for general unittest syntax help.
metadata:
  author: Lawrence Eldridge
---

# ltm-test — Test Lifecycle for claude-ltm

Operational depth for testing claude-ltm. The suite is **stdlib `unittest`**
...
1. **The unittest suite** — `python3 -m unittest discover -s tests` from
   `plugins/ltm`. Correctness of the core, hooks, adapters, CLI, MCP tools.
2. **The recall benchmark** — `python3 bin/ltm eval`. Recall@1/@3, MRR@10,
...
```
/ltm-test run [scope]        run the suite (all, a file, or a TestCase)
/ltm-test create <module>    write a stdlib-unittest test for a core module / hook / adapter
/ltm-test review [path]      QE-standards + anti-pattern review of a test file
/ltm-test audit [path]       advisory leanness audit — deletable / mergeable / redundant tests
/ltm-test bench [backends]   run `ltm eval`; read Recall/MRR/bytes; compare backends
/ltm-test coverage [module]  optional coverage report (stdlib `coverage`, opt-in dep)
```
...
| `bench` | backend spec | Run `ltm eval`, report the metric table, interpret an A/B against the default |
...
§ "Common failure patterns" and propose a fix (leaked `LTM_DATA_DIR`, an
...
2. Pick the fixture shape from [`references/test-data.md`](references/test-data.md):
   ...
   + `os.environ["LTM_DATA_DIR"]` in `setUp`/`tearDown`; anything embedding-touching
...
5. Write to `plugins/ltm/tests/test_<area>.py` (add to an existing file if one
...
signature drifted from the real adapter), leaked global state (`LTM_DATA_DIR` /
...
```bash
cd plugins/ltm
python3 bin/ltm eval --backends hash                                   # zero-dep default
python3 bin/ltm eval --backends "hash,fastembed"                       # A/B the stub vs real model
python3 bin/ltm eval --backends "fastembed,fastembed+float"            # isolate int8 quantization loss
```
...
2. **Measure retrieval changes.** Embeddings, ranking, quantisation, fusion, and
   distillation changes are A/B'd with `ltm eval` before merge. Don't reason about
   recall — measure it.
...
4. **Fixtures are local and self-cleaning.** `tempfile.TemporaryDirectory` +
   `LTM_DATA_DIR` set in `setUp` and restored in `tearDown`; close every `Store`.
...
- [`references/test-data.md`](references/test-data.md) — local fixtures, stub
  ...
  `LTM_DATA_DIR` pattern. No faker, no DB, no network.
...
- [`references/test-leanness-heuristics.md`](references/test-leanness-heuristics.md) —
  ...
  to delete" allowlist.
- [`references/benchmark.md`](references/benchmark.md) — `ltm eval` in depth:
...
Leaked global state. Confirm `LTM_DATA_DIR` (and any `LTM_*` env you set) is
...
### `ltm eval` reports 0.0 across the board
...
`python3 bin/ltm eval --backends hash` as the zero-dep sanity check.
...
- [`memory-recall` skill](../../../plugins/ltm/skills/memory-recall/SKILL.md) —

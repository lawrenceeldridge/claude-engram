...
claude-ltm is a **single Python package** (not a monorepo), so this is a
...
(`plugins/ltm/core`, `plugins/ltm/bin`, `tests/`, `bench/`, `viewer/`). When it
...
| File | Role |
|---|---|
| `config.py` | Resolve config from `userConfig` (`CLAUDE_PLUGIN_OPTION_*`) / `LTM_*` env. |
...
| `mark_consulted.py` | PostToolUse — records that an ltm lookup ran (enables ordering). |
...
| `mcp_server.py` | `ltm-memory` MCP server (`recall`, `search_code`, `get_symbol`, `code_outline`, `search_docs`, `get_doc_section`, `doc_outline`, `index_docs`, `list_projects`). |
| `daemon.py` | Optional resident embedder (keeps the model warm). |
| `ltm` | The CLI — `doctor`, `capture`, `recall`, `core`, `projects`, `prune`, `sweep`, `setup`, `daemon`, `viewer`, `stats`, `drift`, `eval`, `demo`. |
...
## `bench/` — labelled recall benchmark (`ltm eval`)

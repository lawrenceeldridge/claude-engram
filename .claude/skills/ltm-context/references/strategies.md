...
empty.** This is exactly the ordering the repo's own `LTM_ENFORCE` guard steers
...
`recall` (MCP tool) or `ltm recall "<query>"` / `ltm core`. Read the verdict:
...
- **Reading a whole indexed code file to find one symbol.** Use `search_code` +
  `get_symbol`; under `LTM_ENFORCE=strict` reading a large indexed file whole is
  denied outright.
